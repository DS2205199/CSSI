...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a inserire trasferimenti in questa pagina. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title> Aggiungi Trasferimento - Comitato Sicurezza Stradale </title>
    <!------------------------------------------------------------------------>
...
<!-- HTML FINE MENU NEWS -->
  ...
  <!------------------------------------------------------------------------>
  <!-- INIZIO PHP INSERIMENTO SU TABELLA TASFERIMENTI -->
...
$stmt->execute(array($email, $cognome, $nome, $pageName, $indirizzo_ip, $accesso_negato));
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $errors = require '../include/validazioni/validazionetrasferimento.php';
  ...
  $sql = "INSERT INTO trasferimenti (cognome, nome, destinazione, datarichiesta, sede)
  VALUES (?,?,?,?,?)";
  ...
  $stmt->bind_param("sssss", $cognome, $nome, $destinazione, $datarichiesta, $sede);

  // Esecuzione della query
  if ($stmt->execute()) {
  echo '<div id="success-message" class="alert alert-primary" role="alert">'; 
  echo 'Trasferimento aggiunto con successo!';
  echo '</div>';
   echo '<script>
   setTimeout(function() {
...
$stmt->close();
            ...
            echo 'Non hai i privilegi necessari per aggiungere Trasferimenti. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
...
?>
  ...
  <div class="card-header"><center>Inserisci Trasferimento</span></h4></center></div>
  <div class="card-body">
  ...
  <input type="hidden" class="form-control" id="id" name="id" placeholder="ID">
	...
	<div class="row mb-2"> 
  <div class="col">
  <label class="form-label"> Cognome <i class="fa fa-asterisk"></i> </span> </label>
	<select class="form-control dark-input" name="cognome" required "> <?php echo $optionsnomecognome; ?> </select>
  </div>
                
  <div class="col">
  <label class="form-label"> Nome <i class="fa fa-asterisk"></i> </span> </label>
  <select class="form-control dark-input" name="nome" required"> <?php echo $optionsnome; ?> </select>
   </div> </div> </div>
			                   
  <div class="card-footer text-end">
  <button type="submit" class="btn btn-primary"> <i class="fa fa-paper-plane" aria-hidden="true"></i> Invia Trasferimento</button>
  ...
  <div class="card-body">
   <label class="form-label"> Destinazione <i class="fa fa-asterisk"></i> </span> </label>
  <select class="form-control dark-input" name="destinazione" required"> <?php echo $optionsdestinazione; ?> </select>
  </div> </div>
  
  <div class="card mb-4 dark-profile">
  <div class="card-body">
  <label class="form-label"> Data Richiesta <i class="fa fa-asterisk"></i> </span> </label>
  <input type="datetime-local" class="form-control dark-input" id="datarichiesta" name="datarichiesta" placeholder="Data Richiesta" min="2023-01-01T00:00">
  </div> </div>
  
  <div class="card mb-4 dark-profile">
  <div class="card-body">
  <label class="form-label"> Sede <i class="fa fa-asterisk"></i> </span> </label>
  <select class="form-control dark-input" name="sede" required"> <?php echo $optionsedi; ?> </select>
  </div> </div>
  <!-- FINE FORM CAMPI -->
  <!------------------------------------------------------------------------>
  <!-- INZIO JAVASCRIPT-->
