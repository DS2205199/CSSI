...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a inserire video in questa pagina. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title> Aggiungi Video - Comitato Sicurezza Stradale </title>
        <!------------------------------------------------------------------------>
...
<!-- HTML FINE MENU NEWS -->
  ...
  <!-- HTML INZIO FINE LOG DEGLI ACCESSI -->
  <!------------------------------------------------------------------------>
...
if (isset($_POST['submit'])) {
    ...
    $errors = require '../include/validazioni/validazionevideo.php';
    ...
    if (count($errors) > 0) {
        ...
        if ($ruolo === 'Presidente' || $ruolo === 'Segretaria') {
            ...
            $videoTitle = isset($_POST['video_title']) ? mysqli_real_escape_string($conn, $_POST['video_title']) : '';
            $videoType = isset($_POST['video_type']) ? mysqli_real_escape_string($conn, $_POST['video_type']) : '';
            $videoUrl = isset($_POST['video_url']) ? mysqli_real_escape_string($conn, $_POST['video_url']) : '';
            ...
            $publish_date = isset($_POST['publish_date']) ? mysqli_real_escape_string($conn, $_POST['publish_date']) : '';
            $videoFile = isset($_FILES['video_file']) ? $_FILES['video_file'] : '';

            // Verifica se è stato fornito un link di YouTube
            if ($videoType == 'youtube') {
                // Query per inserire il video nel database
                $sql = "INSERT INTO videos (video_title, video_type, video_url, last_name, first_name, publish_date) 
                        VALUES (?, 'youtube', ?, ?, ?, ?)";
                ...
                $stmt->bind_param("sssss", $videoTitle, $videoUrl, $last_name, $first_name, $publish_date);
                ...
                $targetDir = "../src/video/"; // Modifica il percorso della cartella "uploads"
                $targetFile = $targetDir . basename($videoFile["name"]);
                $uploadOk = 1;
                $videoFileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

                // Verifica se il file è un video valido
                $validVideoTypes = array("mp4");
                if (!in_array($videoFileType, $validVideoTypes)) {
                    echo "Il file non è un video valido.";
                    $uploadOk = 0;
                }

                // Carica il file solo se è valido
                if ($uploadOk == 1) {
                    if (move_uploaded_file($videoFile["tmp_name"], $targetFile)) {
                        // Query per inserire il video nel database
                        $sql = "INSERT INTO videos (video_title, video_type, video_file, last_name, first_name, publish_date) 
                                VALUES (?, 'file', ?, ?, ?, ?)";
                        ...
                        // Associazione dei valori ai parametri
                        $stmt->bind_param("sssss", $videoTitle, $targetFile, $last_name, $first_name, $publish_date);
            ...
            echo 'Non hai i privilegi necessari per aggiungere Video. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
...
?>
  ...
  <!-- FINE PHP INSERIMENTO VIDEO -->
  <!------------------------------------------------------------------------>
  <!-- CAMPI FORM -->

<!-- INIZIO FORM REGISTRAZIONE APPARTENENZA -->
  <div class="row">
  <div class="col-lg-9">
  <div class="card w-100 mb-3 dark-profile">
  <div class="card-header"><center>Inserisci Video</span></h4></center></div>
  <div class="card-body">
...
<div class="row mb-2">
                <div class="col">
                  ...
                  <input type="hidden" class="form-control" id="id" name="id" placeholder="ID">
				      ...
				      <div class="row mb-2">
                        <div class="col">
                            <label class="form-label">Titolo Video <i class="fa fa-asterisk"></i></label>
                            <input class="form-control dark-input" placeholder="Titolo Video" name="video_title" required">
                        </div>
                        
                        <div class="col">
                            <label class="form-label">Tipo di Video <i class="fa fa-asterisk"></i></label>
                            <select class="form-control dark-input" name="video_type" required ">
                                <option value="youtube">Link di YouTube</option>
                                <option value="file">Carica un file</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-2">
                        <div class="col">
                            ...
                            <input class="form-control dark-input" name="last_name" required readonly value="<?php echo $_SESSION['cognome'] ?>">
                        </div>
                        <div class="col">
                            <label class="form-label">Nome <i class="fa fa-asterisk"></i></label>
                            <input class="form-control dark-input" name="first_name" required readonly value="<?php echo $_SESSION['nome'] ?>">
                        </div>
                    </div>

                    <div id="youtube-url" class="row mb-2">
                        <div class="col">
                            <label class="form-label">URL del Video di YouTube <i class="fa fa-asterisk"></i></label>
                            <input class="form-control dark-input" name="video_url">
                        </div>
                    </div>

                    <div id="file-upload" class="row mb-2">
                        <div class="col">
                            <label class="form-label">Carica il File Video <i class="fa fa-asterisk"></i></label>
                            <input type="file" class="form-control dark-input" name="video_file">
                        </div>
                    </div>
            ...
            <div class="card-footer text-end">
                <button type="submit" name="submit" class="btn btn-primary">
                    <i class="fa fa-paper-plane" aria-hidden="true"></i> Invia Video
                </button>
				
				<button type="reset" class="btn btn-danger"> <i class="fa fa-trash" aria-hidden="true"></i> Elimina Modulo </button>
    ...
    <div class="col-lg-3">
        <div class="card mb-4 dark-profile">
            <div class="card-header">
                <center> Informazioni Aggiuntive </center>
            </div>
            <div class="card-body">
	
<label class="form-label">Data e Ora di Pubblicazione <i class="fa fa-asterisk"></i> </label>
                <input type="datetime-local" name="publish_date" placeholder="Data e Ora di Pubblicazione" class="form-control dark-input" required">
            </div>
        </div>
    </div>
</div>

 <!-- FINE FORM REGISTRAZIONE APPARTENENZA -->
 ...
 <!-- INZIO JAVASCRIPT-->
  ...
  <script>
    // Nascondi/visualizza i campi in base alla selezione del tipo di video
