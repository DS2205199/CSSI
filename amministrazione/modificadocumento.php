...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a modifica le informazioni del Documento. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title> Modifica Documento - Comitato Sicurezza Stradale </title>
	<!------------------------------------------------------------------------>
...
<!-- HTML FINE MENU NEWS -->
  ...
  <!------------------------------------------------------------------------>
  <!-- INIZIO PHP MODIFICA SU TABELLA DOCUMENTI -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $ID = isset($_POST['ID']) ? mysqli_real_escape_string($conn, $_POST['ID']) : '';
            ...
            // Filtro e validazione dei dati di input
            $nomedocumento = isset($_POST['nomedocumento']) ? filter_var($_POST['nomedocumento'], FILTER_SANITIZE_STRING) : '';
            $linkdocumento = isset($_POST['linkdocumento']) ? filter_var($_POST['linkdocumento'], FILTER_SANITIZE_URL) : '';
            $dataeoradinserimento = isset($_POST['dataeoradinserimento']) ? mysqli_real_escape_string($conn, $_POST['dataeoradinserimento']) : '';
            $registratoda = isset($_POST['registratoda']) ? filter_var($_POST['registratoda'], FILTER_SANITIZE_STRING) : '';
            $scaricadocumento = isset($_POST['scaricadocumento']) ? mysqli_real_escape_string($conn, $_POST['scaricadocumento']) : '';
    
    // Esecuzione dell'aggiornamento dei dati nella tabella utilizzando uno statement preparato
    $stmt = $conn->prepare("UPDATE documenti SET nomedocumento = ?, linkdocumento = ?, scaricadocumento = ?, registratoda = ?, dataeoradinserimento = ? WHERE ID = ?");
    $stmt->bind_param("sssssi", $nomedocumento, $linkdocumento, $scaricadocumento, $registratoda, $dataeoradinserimento, $ID);
    ...
    $stmt->close();
            ...
            echo "Non hai i privilegi necessari per Modificare i Documenti. Si prega di contattare amministratore per ulteriori informazioni.";
            echo '</div>';
        }
    }
}
...
// Recupero dei dati dalla tabella utilizzando uno statement preparato
...
?>
  ...
  <!------------------------------------------------------------------------>
  <!-- FINE PHP MODIFICA SU TABELLA DOCUMENTI -->
  ...
  <div class="card-header"><center>Inserimento Documento</center></div>
  <div class="card-body">
  <h5 class="card-title"><center><b>Nota importante: I campi contrassegnati con un asterisco (<i class="fa fa-asterisk"></i> ) sono obbligatori. Si prega di compilare tutti i campi richiesti correttamente.</center></h5></b>
              ...
              <div class="row mb-2">
                ...
                <div class="col">
                  <class="form-label">Nome Documento <i class="fa fa-asterisk"></i> </label>
                  <input class="form-control dark-input" name="nomedocumento" value="<?php echo "" . $row["nomedocumento"], "" ?>" required">
                </div>
               
			   <div class="col">
                  <class="form-label">Link Documento <i class="fa fa-asterisk"></i> </label>
                  <input class="form-control dark-input" name="linkdocumento" value="<?php echo "" . $row["linkdocumento"], "" ?>" required">
                </div>
              </div>
              ...
              <div class="row mb-2">
                <div class="col">
                 <class="form-label"> Scarica Documento <i class="fa fa-asterisk"></i> </label>
                  <input type="text" class="form-control dark-input " id="scaricadocumento" name="scaricadocumento" placeholder="scaricadocumento" value="<?php echo "" . $row["scaricadocumento"], "" ?>" required">
                </div>
                
				<div class="col">
                  <class="form-label"> Registrato Da <i class="fa fa-asterisk"></i> </label>
                  <input class="form-control dark-input" name="registratoda" value="<?php echo $_SESSION['cognome'] . ' ' . $_SESSION['nome']; ?>" readonly>
				  
              </div>
                <div class="col">
                  <class="form-label">Data e Ora di Inserimento <i class="fa fa-asterisk"></i> </label>
                  <input type="text" class="form-control dark-input" id="dataeoradinserimento" name="dataeoradinserimento" placeholder="Data e Ora di Inserimento" value="<?php echo "" . $row["dataeoradinserimento"], "" ?>" required">
                </div>
              </div>
              </div>

<div class="card-footer text-end">
    <button type="submit" class="btn btn-primary"> <i class='fas fa-pencil-alt'></i> Aggiorna Documento</button>
