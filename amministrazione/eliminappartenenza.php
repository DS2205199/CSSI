...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a eliminare le appartenenze. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title> Elimina Appartenenza - Comitato Sicurezza Stradale </title>
   <!------------------------------------------------------------------------>
    <!-- CSS ESTERNI -->
...
if (isset($_GET['id'])) {
    ...
    $sql = "DELETE FROM appartenenze WHERE ID = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);

    if ($stmt->execute()) {
        echo '<div id="success-message" class="alert alert-primary" role="alert">'; 
        echo 'Appartenenza rimossa con successo.';
                echo '</div>';
    echo '<script>
   setTimeout(function() {
...
echo '<div id="error-message" class="alert alert-danger" role="alert">';
            echo 'Non hai i privilegi necessari per eliminare la appartenenza. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
