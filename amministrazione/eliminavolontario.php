...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a eliminare i Volontari. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title>Elimina Volontario - Comitato Sicurezza Stradale </title>
     <!------------------------------------------------------------------------>
...
<!-- HTML FINE MENU NEWS -->
  ...
  <!------------------------------------------------------------------------>
  <!-- INIZIO ELIMINAZIONE VOLONTARIO -->
...
if (isset($_GET['id'])) {
    ...
    $sql = "DELETE FROM volontari WHERE ID = ?";
    ...
    if ($stmt->execute()) {
      ...
      echo 'Volontario eliminato dai Soci Attvi';
      echo '</div>';
    echo '<script>
   setTimeout(function() {
...
echo '<div id="error-message" class="alert alert-danger" role="alert">';
            echo 'Non hai i privilegi necessari per eliminare il Volontario. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
