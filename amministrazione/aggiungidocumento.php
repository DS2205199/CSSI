...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a inserire documenti in questa pagina. Se hai bisogno di assistenza, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title>Aggiungi Documento - Comitato Sicurezza Stradale</title>
...
$conn = new mysqli($db_host, $db_user, $db_pass, $db_name);
  ...
  <!------------------------------------------------------------------------>
  <!-- INZIO PHP INSERIMENTO SU TABELLA DOCUMENTI -->
...
$conn = new mysqli($db_host, $db_user, $db_pass, $db_name);
  ...
  $scaricadocumento = isset($_POST['scaricadocumento']) ? mysqli_real_escape_string($conn, $_POST['scaricadocumento']) : '';
            ...
            // Creazione della query di inserimento con segnaposti dei parametri
            $sql = "INSERT INTO documenti (nomedocumento, linkdocumento, dataeoradinserimento, registratoda, scaricadocumento)
            VALUES (?, ?, ?, ?, ?)";
            ...
            $stmt->bind_param("sssss", $nomedocumento, $linkdocumento, $dataeoradinserimento, $registratoda, $scaricadocumento);

            // Esecuzione della query
            if ($stmt->execute()) {
                echo '<div id="success-message" class="alert alert-primary" role="alert">';
                echo 'Il documento è stato aggiunto al database con successo e inviato!';
                echo '</div>';
                echo '<script>
                setTimeout(function() {
...
?>
  ...
  <!-- FINE PHP INSERIMENTO SU TABELLA DOCUMENTI -->
  <!------------------------------------------------------------------------>
  <!-- INZIO FORM CAMPI -->
  ...
  <div class="card-header"><center>Inserimento Documenti</center></div>
  <div class="card-body">
  ...
  <div class="col-lg-6">
  <div class="mb-3">
  <label class="form-label"> Nome del Documento <i class="fa fa-asterisk"></i></label>
	<input type="text" class="form-control dark-input" name="nomedocumento" placeholder="Nome Documento" required >
  </div> </div>

  <div class="col-lg-6">
  <div class="mb-3">
  <label class="form-label"> Link del Documento <i class="fa fa-asterisk"></i> </span> </label>
  <input class="form-control dark-input" name="linkdocumento" placeholder="Link Documento" required >
  </div> </div> 

  <div class="col-lg-15">
  <div class="mb-3">
  <label class="form-label"> Link per Download <i class="fa fa-asterisk"></i> </span></label>
  <input type="text" name="scaricadocumento" placeholder="Scarica Documento" class="form-control dark-input" required >
  </div> </div> </div> </div>
  
  <div class="card-footer text-end">

  <button type="submit" class="btn btn-primary"> <i class="fa fa-paper-plane" aria-hidden="true"></i> Invia Documento</button>
  <button type="reset" class="btn btn-danger">  <i class="fa fa-trash"></i> Elimina Modulo</button>
  </div> </div> </div>
  ...
  <div class="card-body">
  <label class="form-label">Data e Ora di Inserimento <i class="fa fa-asterisk"></i> </span> </label>
  <input type="datetime-local" name="dataeoradinserimento" placeholder="Data e Ora di Inserimento" class="form-control dark-input" >
  </div> </div>
  
  <div class="card mb-4 dark-profile">
  <div class="card-body">
  <label class="form-label"> Registrato Da <i class="fa fa-asterisk"></i> </span> </label>
  <input type="text" name="registratoda" placeholder="Registrato Da" class="form-control dark-input" value="<?php echo $_SESSION['cognome'] . ' ' . $_SESSION['nome']; ?>" readonly>
  </div> </div>
