...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a eliminare le attivita. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title>Elimina Attivita - Comitato Sicurezza Stradale </title>
    <!------------------------------------------------------------------------>
...
// Verifica se il parametro ID dell'attività è stato passato tramite GET o POST
if (isset($_GET['id'])) {
	        
			if ($ruolo === 'Presidente' || $ruolo === 'Segretaria') {

  // Prepara l'ID dell'attività per evitare SQL injection
  $attivitaID = $conn->real_escape_string($_GET['id']);

  // Elimina le notifiche associate all'attività
  $deleteNotificheQuery = "DELETE FROM notifiche WHERE id = ?";
  $stmt = $conn->prepare($deleteNotificheQuery);
  $stmt->bind_param("s", $attivitaID);

  if (!$stmt->execute()) {
      echo "Errore durante l'eliminazione delle notifiche: " . $stmt->error;
      exit;
  }
  ...
  // Elimina l'attività
  $deleteAttivitaQuery = "DELETE FROM attivita WHERE ID = ?";
  $stmt = $conn->prepare($deleteAttivitaQuery);
  $stmt->bind_param("s", $attivitaID);
...
echo '<div id="error-message" class="alert alert-danger" role="alert">';
            echo 'Non hai i privilegi necessari per eliminare le Attività. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
