...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a visualizzare i Video. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="it">
  <head>
    ...
    <!-- TITOLO DELLA PAGINA -->
    <title> Guarda Video - Comitato Sicurezza Stradale </title>
...
if (isset($_GET['id'])) {
  // Pulisci e valida l'ID dell'appartenenza
  $id = isset($_GET['id']) ? intval($_GET['id']) : 0;

  // Effettua la connessione al database e esegui la query per ottenere il link di YouTube e il nome del video caricato
  // Sostituisci le seguenti righe con il tuo codice per la connessione al database

  // Crea la connessione
  $conn = new mysqli($servername, $username, $password, $dbname);

  // Verifica eventuali errori nella connessione
  if ($conn->connect_error) {
    die("Connessione fallita: " . $conn->connect_error);
  }

  // Esegui la query per ottenere il link di YouTube e il nome del video caricato utilizzando una dichiarazione preparata
  $stmt = $conn->prepare("SELECT video_url, video_file FROM videos WHERE id = ?");
  ...
  if ($result->num_rows > 0) {
    ...
    // Visualizza il video di YouTube se è stato fornito un link
    if (!empty($video_url)) {
      // Ottieni l'ID del video da YouTube
      $videoId = getYoutubeVideoId($video_url);

      // Visualizza il video di YouTube utilizzando l'ID ottenuto
      echo '<iframe width="1560" height="720" src="https://www.youtube.com/embed/' . $videoId . '" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
    }

    // Visualizza il video caricato se è stato fornito un file
    if (!empty($video_file)) {
      echo '<video width="1560" height="720" controls>
              <source src="' . $video_file . '" type="video/mp4">
            </video>';
    }
  } else {
    // Nessun video trovato con l'ID specificato
    echo "Video non trovato.";
  }
} else {
  // Nessun ID fornito nella query string
  echo "ID del video non specificato.";
}

// Funzione per ottenere l'ID del video da un link di YouTube
