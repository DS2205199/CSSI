...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a inserire estensioni in questa pagina. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  ...
  <!------------------------------------------------------------------------>
  <!-- INZIO PHP INSERIMENTO SU TABELLA APPARTENENZE -->
...
$stmt->execute(array($email, $cognome, $nome, $pageName, $indirizzo_ip, $accesso_negato));
  ...
  $sql = "INSERT INTO appartenenze (cognome, nome, inizio, fine, tipo, sede)
  VALUES (?, ?, ?, ?, ?, ?)";
  ...
  $stmt->bind_param("ssssss", $cognome, $nome, $inizio, $fine, $tipo, $sede);

  // Esecuzione della query
  if ($stmt->execute()) {
  echo '<div id="success-message" class="alert alert-primary" role="alert">';
  echo 'Appartenenza è stata aggiunta con successo!';
  echo '</div>';
  echo '<script>
  setTimeout(function() {
  ...
  $stmt->close();
            ...
            echo 'Non hai i privilegi necessari per aggiungere Estensioni. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
...
$conn->close();
?>
  <!-- FINE PHP INSERIMENTO SU TABELLA APPARTENENZE -->
  <!------------------------------------------------------------------------>
  ...
  $options .= "<option value='$nomesede'>$nomesede</option>"; } }
   ...
   // Esecuzione della query per ottenere i dati dal database
   $sql1 = "SELECT cognome FROM volontari";
   ...
   $cognome = $row["cognome"];
   $options1 .= "<option value='$cognome'>$cognome</option>"; } }


  // Esecuzione della query per ottenere i dati dal database
   $sql2 = "SELECT nome FROM volontari";
   ...
   $nome = $row["nome"];
   $options2 .= "<option value='$nome'>$nome</option>"; } }
  ...
  <!-- FINE PHP MENU A TENDINA SU TABELLA SEDI -->
  <!------------------------------------------------------------------------>
  <!-- INZIO FORM CAMPI -->
  ...
  <div class="card-header"><center>Inserisci appartenenza</span></h4></center></div>
  <div class="card-body">
  ...
  <select class="form-control dark-input" name="cognome" required"> <?php echo $options1; ?> </select>

  </div>
                
  <div class="col">
  <label class="form-label"> Nome <i class="fa fa-asterisk"></i></label>
  <select class="form-control dark-input" name="nome" required"> <?php echo $options2; ?> </select>
  </div> </div> </div>
