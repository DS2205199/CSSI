...
if (isset($_SESSION["last_activity"])) {
    // Imposta il limite di inattività desiderato in secondi (ad esempio, 10 minuti)
    $inactivity_limit = 600; // 10 minuti in secondi
...
// Recupera le variabili di sessione dell'utente
$ID = $_SESSION["ID"];
$nome = $_SESSION["nome"];
...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a inserire News in questa pagina. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<head>
  ...
  <title>Aggiungi News - Comitato Sicurezza Stradale</title>
 ...
 <li class="nav-item">
  ...
  <!------------------------------------------------------------------------>
  <!-- INZIO PHP INSERIMENTO SU TABELLA NEWS -->
...
$stmt->execute(array($email, $cognome, $nome, $pageName, $indirizzo_ip, $accesso_negato));
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $errors = require '../include/validazioni/validazionenews.php';
  ...
  $nomenews = isset($_POST['nomenews']) ? mysqli_real_escape_string($conn, $_POST['nomenews']) : '';
  ...
  $sql = "INSERT INTO news (cognome, nome, nomenews, publish_date, descrizione)
  VALUES (?, ?, ?, ?, ?)";
  ...
  $stmt->bind_param("sssss", $cognome, $nome, $nomenews, $publish_date, $descrizione);
...
$stmt->close();
            ...
            echo 'Non hai i privilegi necessari per aggiungere News. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
...
?>
  ...
  <div class="card-header"><center>Inserisci News </center></div>
  <div class="card-body">
  ...
  <div class="mb-3">
  <label class="form-label"> Titolo della Notizia <i class="fa fa-asterisk"></i> </label>
  <input type="text" name="nomenews" placeholder="Titolo News" class="form-control dark-input" required >
  </div>
  ...
  <input type="text" name="cognome" placeholder="Cognome" class="form-control dark-input" value="<?php echo $_SESSION['cognome']; ?>" required readonly>
  </div> </div>

  <div class="col-lg-6">
  <div class="mb-3">
  <label class="form-label">Nome <i class="fa fa-asterisk"></i> </label>
  <input type="text" name="nome" placeholder="Nome" class="form-control dark-input" value="<?php echo $_SESSION['nome']; ?>" required readonly>
  </div> </div> </div>
  
  <div class="row">
  <div class="col-lg-15">
  <div class="mb-3">
  <label class="form-label"> Inserisci News <i class="fa fa-asterisk"></i></label>
  <textarea class="form-control dark-input" name="descrizione" id="descrizione" rows="3" required > </textarea>
  </div> </div> </div> </div>

  <div class="card-footer text-end">
  <button type="submit" class="btn btn-primary"> <i class="fa fa-paper-plane" aria-hidden="true"></i> Invia News</button>
  ...
  <div class="card-body">
  <label class="form-label">Data di Pubblicazione <i class="fa fa-asterisk"></i></label>
  <input type="date" name="publish_date" placeholder="Data di Pubblicazione" class="form-control dark-input" required min="2023-01-01T00:00">
  </div> </div> </div>
