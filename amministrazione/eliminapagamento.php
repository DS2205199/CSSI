...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a eliminare i pagamenti. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title>Elimina Pagamento - Comitato Sicurezza Stradale </title>
   <!------------------------------------------------------------------------>
    <!-- CSS ESTERNI -->
...
<!-- HTML FINE MENU NEWS -->
  ...
  <!------------------------------------------------------------------------>
  <!-- PHP INIZIO ELIMINAZIONE PAGAMENTO -->
...
$sql = "DELETE FROM quotepagate WHERE ID = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    
  if ($stmt->execute()) {
  echo '<div id="success-message" class="alert alert-primary" role="alert">'; 
  echo 'Pagamento eliminato con successo dal database';
  echo '</div>';
    echo '<script>
   setTimeout(function() {
...
echo '<div id="error-message" class="alert alert-danger" role="alert">';
            echo 'Non hai i privilegi necessari per eliminare i Pagamenti. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
...
$conn->close();
?>
  <!-- PHP FINE ELIMINAZIONE PAGAMENTO -->
<!------------------------------------------------------------------------>
