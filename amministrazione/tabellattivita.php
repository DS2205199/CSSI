...
<html lang="it">
  <head>
    ...
    <!-- TITOLO DELLA PAGINA -->
    <title>Tabella Attivita - Comitato Sicurezza Stradale </title>
...
<div class="alert alert-info" role="alert">
  <center><b> Benvenuto nella pagina dedicata alla Gestione delle Attività </b>
</div>

<form method="GET" action="" class="mb-3">
    <div class="input-group mb-2">
<a href="insertattivita.php" class="btn btn-outline-primary rounded mr-2">Aggiungi Attività</a>

<input type="text" name="nomeattivita" class="form-control rounded mr-3" placeholder="Nome Attività">
        <input type="text" name="dataeoradelservizio" class="form-control rounded mr-3" placeholder="Data e Ora del Servizio">
        <input type="text" name="posizioneservizio" class="form-control rounded mr-3" placeholder="Posizione Servizio">
...
if (isset($_GET['submit'])) {
    $nomeattivita = $_GET['nomeattivita'];
    $dataeoradelservizio = $_GET['dataeoradelservizio'];
	$posizioneservizio = $_GET['posizioneservizio'];

    // Effettua la query per selezionare i volontari corrispondenti alla ricerca
    $sql = "SELECT * FROM attivita WHERE nomeattivita LIKE '%$nomeattivita%' AND dataeoradelservizio LIKE '%$dataeoradelservizio%' AND posizioneservizio LIKE '%$posizioneservizio%'";
} else {
...
if ($result->num_rows > 0) {
        ...
        <th><center>Nome Attività</th>
				<th><center>Posizione Servizio</th>
				<th><center>Nome Referente</th>
				<th><center>Area di Intervento</th>
				<th><center>Aperto a</th>
        <th><center>Data e Ora Servizio</center></th>
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>";
                            ...
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["nomereferente"] . "</td>";
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["areadintervento"] . "</td>";
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["apertoa"] . "</td>";
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["dataeoradelservizio"] . "</td>";
                            
							echo "<td class='cell100 column2 align-middle text-center'>";
                            echo "<a href='guardattivita.php?id=" . $row['ID'] . "'>";
                            echo "<i class='fa fa-eye'></i></a>";
							
							echo "<td class='cell100 column2 align-middle text-center'>";
                            echo "<a href='modificattivita.php?id=" . $row['ID'] . "'>";
                            echo "<i class='fas fa-pencil-alt'></i></a>";                          
                            
                            echo "<td class='cell100 column2 align-middle text-center'>";
                            echo "<a href='eliminattivita.php?id=" . $row['ID'] . "'>";
