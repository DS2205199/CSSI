...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a eliminare i Trasferimenti. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title> Elimina Trasferimento - Comitato Sicurezza Stradale </title>
    <!------------------------------------------------------------------------>
    <!-- CSS ESTERNI -->
...
<!-- HTML FINE MENU NEWS -->
  ...
  <!------------------------------------------------------------------------>
  <!-- INZIO ELIMINAZIONE TRASFERIMENTI -->
  ...
  $sql = "DELETE FROM trasferimenti WHERE ID = ?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("i", $id);

  if ($stmt->execute()) {
  echo '<div id="success-message" class="alert alert-primary" role="alert">'; 
  echo 'Il Trasferimento è stato rimosso con successo.';
   echo '</div>';
    echo '<script>
   setTimeout(function() {
...
echo '<div id="error-message" class="alert alert-danger" role="alert">';
            echo 'Non hai i privilegi necessari per eliminare il Trasferimento. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
...
$conn->close();
?>
  <!-- FINE ELIMINAZIONE TRASFERIMENTI -->
<!------------------------------------------------------------------------>
