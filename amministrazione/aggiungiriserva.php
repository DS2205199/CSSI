...
<div class="message">
  ...
  <center> <p> Mi dispiace, ma non sei autorizzato a inserire Riserve in questa pagina. Se hai bisogno di assistenza o se per te e un errore, contatta amministratore del sistema.</p></center>
  <br>
...
<html lang="en">
  <head>
    ...
    <title> Aggiungi Riserva - Comitato Sicurezza Stradale </title>
   <!------------------------------------------------------------------------>
...
<!-- HTML FINE MENU NEWS -->
  ...
  <ul class="nav flex-column">
  <li class="nav-item">
  <a class="nav-link" href="tabellaaccessi.php">Visualizza Log degli Accessi</a>
  </li> </ul> </div> </li>
  ...
  <!------------------------------------------------------------------------>
  <!-- INZIO PHP INSERIMENTO SU TABELLA RISERVA -->
...
$stmt->execute(array($email, $cognome, $nome, $pageName, $indirizzo_ip, $accesso_negato));
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $errors = require '../include/validazioni/validazioneriserva.php';
  ...
  $fine = isset($_POST['fine']) ? mysqli_real_escape_string($conn, $_POST['fine']) : '';
   ...
   $sql = "INSERT INTO riserve (cognome, nome, inizio, fine)
   VALUES (?,?,?,?)";
   ...
   $stmt->bind_param("ssss", $cognome, $nome, $inizio, $fine);

   // Esecuzione della query
   if ($stmt->execute()) {
   echo '<div id="success-message" class="alert alert-primary" role="alert">'; 
   echo 'Riserva aggiunta con successo!';
   echo '</div>';
   echo '<script>
   setTimeout(function() {
...
$stmt->close();
            ...
            echo 'Non hai i privilegi necessari per aggiungere Riserve. Si prega di contattare amministratore per ulteriori informazioni.';
            echo '</div>';
			echo '<script>
                setTimeout(function() {
...
?>
  ...
  <!------------------------------------------------------------------------>
  <!-- MENU A TENDINA DELLE TABELLE INIZIO -->
  ...
  <!-- MENU A TENDINA DELLE TABELLE FINE -->
  <!------------------------------------------------------------------------>
  <!-- INZIO FORM CAMPI -->
  ...
  <div class="card-header"><center>Inserisci Riserva</span></h4></center></div>
  <div class="card-body">
  ...
  <select class="form-control dark-input" name="cognome" placeholder="Cognome" required"> <?php echo $optionsnomecognome ?> </select>
  </div> </div> 
                
	<div class="col-lg-6">
  <div class="mb-3">
  <label class="form-label"> Nome <i class="fa fa-asterisk"></i> </span> </label>
  <select class="form-control dark-input" name="nome" placeholder="Nome" required"> <?php echo $optionsnome ?> </select>
  </div>  </div> </div> </div> 
			  


  <div class="card-footer text-end">
  <button type="submit" class="btn btn-primary"><i class="fa fa-paper-plane" aria-hidden="true"></i>  Aggiungi Riserva</button>
  ...
  <div class="card-body">
  <label class="form-label">Inizio <i class="fa fa-asterisk"></i> </span> </label>
  <input type="datetime-local" class="form-control dark-input" id="inizio" name="inizio" placeholder="Inizio" required oninvalid="this.setCustomValidity('Inserisci una data di inizio valida.')" oninput="this.setCustomValidity('')" min="2023-01-01T00:00">
  </div>  </div>
  
  <div class="card mb-4 dark-profile">
  <div class="card-body">
  <label class="form-label">Fine <i class="fa fa-asterisk"></i> </span></label>
  <input type="datetime-local" class="form-control dark-input" id="fine" name="fine" placeholder="Fine" required oninvalid="this.setCustomValidity('Inserisci una data di fine valida.')" oninput="this.setCustomValidity('')" min="2023-01-01T00:00">
   </div>  </div>
