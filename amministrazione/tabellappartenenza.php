...
<html lang="it">
  <head>
    ...
    <!-- TITOLO DELLA PAGINA -->
    <title> Tabella Appartenenza - Comitato Sicurezza Stradale </title>
...
<div class="alert alert-info" role="alert">
  <center><b>  Da questa pagina potrai visionare lo stato delle estensioni. </b>    
    </div>
</div>

<form method="GET" action="" class="mb-3">
    <div class="input-group mb-2">
	<a href="aggiungiestensioni.php" class="btn btn-outline-primary rounded mr-2" >Aggiungi Documento</a>
	
<input type="text" name="cognome" class="form-control rounded mr-3" placeholder="Cognome">
<input type="text" name="nome" class="form-control rounded mr-3" placeholder="Nome">
...
if (isset($_GET['submit'])) {
   ...
   $fine = $_GET['fine'];
    ...
    $sql = "SELECT * FROM appartenenze WHERE cognome LIKE '%$cognome%' AND nome LIKE '%$nome%' AND inizio LIKE '%$inizio%' AND fine LIKE '%$fine%'";
} else {
    // Se non è stata inviata la ricerca, seleziona tutti i volontari
    $sql = "SELECT * FROM appartenenze";
}
...
if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>";
                              ...
                              echo "<a href='/include/abilita_estensioni.php?id=" . $row["ID"] . "&azione=disabilita'>";
                              echo "<span class='badge bg-success'>Disabilita</button>";
                              echo "</a>";
                              } else {
                              echo "<a href='/include/abilita_estensioni.php?id=" . $row["ID"] . "&azione=abilita'>";
                            ...
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["tipo"] . "</td>";
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["sede"] . "</td>";

							echo "<td class='cell100 column2 align-middle text-center'>";
							echo "<a href='guardaappartenenza.php?id=" . $row["ID"] . "'>";
