...
<html lang="it">
  <head>
    ...
    <!-- TITOLO DELLA PAGINA -->
    <title> Tabella Trasferimenti - Comitato Sicurezza Stradale </title>
...
<div class="alert alert-info" role="alert">
  <center><b> Da questa pagina potrai visionare lo stato dei Trasferimenti. </b>
  <p>
</div>
...
<a href="aggiungitrasferimento.php" class="btn btn-outline-primary rounded mr-2">Aggiungi Trasferimento</a>
        ...
        <input type="text" name="destinazione" class="form-control rounded mr-3" placeholder="Destinazione">
		<input type="text" name="datarichiesta" class="form-control rounded mr-3" placeholder="Data Richiesta">
		<input type="text" name="sede" class="form-control rounded mr-3" placeholder="Sede">
...
if (isset($_GET['submit'])) {
    ...
    $sql = "SELECT * FROM trasferimenti WHERE cognome LIKE '%$cognome%' AND nome LIKE '%$nome%' AND destinazione LIKE '%$destinazione%' AND datarichiesta LIKE '%$datarichiesta%' AND sede LIKE '%$sede%'";
} else {
...
echo "<table id='tabella'>
				...
				<th><center>Nome</th>
				<th><center>Destinazione</th>
				<th><center>Data Richiesta</th>
				<th><center>Sede</th>
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>";
                            ...
                            echo "<tr>";
                              ...
                              echo "<a href='/include/abilita_trasferimento.php?id=" . $row["ID"] . "&azione=disabilita'>";
                              echo "<span class='badge bg-success'>Disabilita</button>";
                              echo "</a>";
                              } else {
                              echo "<a href='/include/abilita_trasferimento.php?id=" . $row["ID"] . "&azione=abilita'>";
                            ...
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["destinazione"] . "</td>";
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["datarichiesta"] . "</td>";
                            echo "<td class='cell100 column2 align-middle text-center '>" . $row["sede"] . "</td>";

							echo "<td class='cell100 column2 align-middle text-center'>";
							echo "<a href='guardatrasferimento.php?id=" . $row["ID"] . "'>";
							echo "<i class='fa fa-eye'></i></a>";
							
							echo "<td class='cell100 column2 align-middle text-center'>";
							echo "<a href='modificatrasferimento.php?id=" . $row["ID"] . "&nome=" . urlencode($row["nome"]) . "&cognome=" . urlencode($row["cognome"]) . "'>";
							echo "<i class='fas fa-pencil-alt'></i></a>";     
							
							echo "<td class='cell100 column2 align-middle text-center'>";
							echo "<a href='eliminatrasferimento.php?id=" . $row["ID"] . "'>";
							...
							echo "<tr><td colspan='12'> Siamo spiacenti, ma non è stato possibile trovare alcun trasferimento corrispondente. </td></tr>";
							}
							?>
