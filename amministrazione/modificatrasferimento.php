...
<html lang="en">
  <head>
    ...
    <title> Modifica Trasferimento - Comitato Sicurezza Stradale </title>
    <!------------------------------------------------------------------------>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (count($errors) > 0) {
        ...
        if ($ruolo === 'Presidente' || $ruolo === 'Segretaria') {
            ...
            $destinazione = isset($_POST['destinazione']) ? mysqli_real_escape_string($conn, $_POST['destinazione']) : '';
            $datarichiesta = isset($_POST['datarichiesta']) ? mysqli_real_escape_string($conn, $_POST['datarichiesta']) : '';
            $sede = isset($_POST['sede']) ? mysqli_real_escape_string($conn, $_POST['sede']) : '';

            // Preparazione dell'istruzione UPDATE con i parametri utilizzando prepared statement
            $query = "UPDATE trasferimenti SET cognome = ?, nome = ?, destinazione = ?, datarichiesta = ?, sede = ? WHERE ID = ?";
            ...
            if ($stmt === false) {
                ...
                // Bind dei parametri e esecuzione dell'istruzione
                $stmt->bind_param("sssssi", $cognome, $nome, $destinazione, $datarichiesta, $sede, $ID);
                $result = $stmt->execute();

                if ($result) {
                    echo '<div id="success-message" class="alert alert-primary" role="alert">';
                    echo 'Trasferimento Aggiornato con Successo!';
                    echo '</div>';
                    echo '<script>
                        setTimeout(function() {
...
?>
  ...
  <!-- FINE PHP MODIFICA SU TABELLA TRASFERIMENTI -->
  <!------------------------------------------------------------------------>
  <!-- INIZIO PHP VISUALIZIONE SU FORM -->
<?php
$conn = new mysqli($servername, $username, $password, $dbname);
...
// Preparazione dell'istruzione SELECT con il parametro utilizzando prepared statement
$query = "SELECT * FROM trasferimenti WHERE ID = ?";
$stmt = $conn->prepare($query);
...
// Bind del parametro e esecuzione dell'istruzione
$stmt->bind_param("i", $id);
$stmt->execute();

// Recupero dei dati dal risultato del prepared statement
...
?>
  ...
  $optionsnome .= "<option value='$nome'>$nome</option>"; } }
    ...
    // Esecuzione della query per ottenere i dati delle descrizioni dal database
    $sqòsedi = "SELECT nomesede FROM sedi";
    $resultsedi = $conn->query($sqòsedi);
    
    $optionsdestinazione = '';
    
    if ($resultsedi->num_rows > 0) {
        
    // Ciclo sui risultati della query e costruzione delle opzioni del menu a tendina per le descrizioni
    while ($rowsedi = $resultsedi->fetch_assoc()) {
    $destinazione = $rowsedi["nomesede"];
    $optionsdestinazione .= "<option value='$destinazione'>$destinazione</option>"; } }

    // Esecuzione della query per ottenere i dati delle descrizioni dal database
    $sqòsedi1 = "SELECT nomesede FROM sedi";
    $resultsedi1 = $conn->query($sqòsedi1);
    ...
    // Ciclo sui risultati della query e costruzione delle opzioni del menu a tendina per le descrizioni
    while ($rowsedi1 = $resultsedi1->fetch_assoc()) {
    $sede = $rowsedi1["nomesede"];
    $optionsedi .= "<option value='$sede'>$sede</option>"; } }
  ...
  <div class="card-header"><center>Modifica Trasferimento di: <?php echo " " . $row["nome"],  " "  . $row["cognome"] . "<br>";;?></span></h4></center></div>
  <div class="card-body">
  <h5 class="card-title"><center><b>Nota importante: I campi contrassegnati con un asterisco (<i class="fa fa-asterisk"></i> ) sono obbligatori. Si prega di compilare tutti i campi richiesti correttamente.</center></h5></b>
              ...
              <div class="row mb-2">
                ...
                <div class="col">
                  ...
                  <input class="form-control dark-input" name="cognome" readonly required value="<?php echo "" . $row["cognome"], "" ?>">
                </div>
                <div class="col">
                  <label for="nome" class="form-label">Nome</label>
                  <input class="form-control dark-input" name="nome" readonly required value="<?php echo "" . $row["nome"], "" ?>">
                </div>  
                  </div>
...
<div class="card-footer text-end">
    <button type="submit" class="btn btn-primary">  <i class="fa fa-paper-plane" aria-hidden="true"></i> Aggiorna Trasferimento </button>
...
<label for="destinazione" class="form-label">Destinazione</label>
                 <select class="form-control dark-input" id="destinazione" name="destinazione" placeholder="Destinazione" required value="<?php echo "" . $row["destinazione"], "" ?>"> <?php echo $optionsdestinazione; ?> </select>
                </div>  </div>

<div class="card mb-4 dark-profile">
  <div class="card-body">
 <label for="datarichiesta" class="form-label">Data Richiesta</label>
                  <input type="datetime-local" class="form-control dark-input" id="datarichiesta" name="datarichiesta" required placeholder="Data Richiesta" value="<?php echo "" . $row["datarichiesta"], "" ?>" min="2023-01-01T00:00">
                  </div> </div>

<div class="card mb-4 dark-profile">
  <div class="card-body">
<label for="sede" class="form-label">Sede</label>
                  <select class="form-control dark-input" id="sede" name="sede" required placeholder="Sede" value="<?php echo "" . $row["sede"], "" ?>"> <?php echo $optionsedi; ?> </select>
                  </div> </div>
  <!-- FINE FORM CAMPI -->
  <!------------------------------------------------------------------------>
  <!-- INZIO JAVASCRIPT-->
